...
def algorithm_SS(Id,final,data_stock,T,T_1,rule_data):
    ...
    # data_stock.to_csv('./data_stock.csv', encoding='utf_8_sig')
    #++++++++++++++++++++++++++++++++++++++++++++++++>与redis进行交互
...
def main_function(Id,data_stock,data_final,today_date):
...
def main(redis_key,wh_code,today_date_str):
    ...
    #-------------------------------------------------->安全库存与预测修正
    data_final                      = algorithm_SS(redis_key,final,data_stock,today_date, T_1,rule_data)
    # data_final.to_csv('./data_final.csv',encoding='utf_8_sig')
    AI_suggestion                   = main_function(redis_key,data_stock, data_final,today_date)
