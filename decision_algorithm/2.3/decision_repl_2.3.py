...
def main_function(Id,wh_code,data_stock,data_final,today_date):
    ...
    def revised_suggest(x):
        if (x['Suggestion_qty']+x['Inventory']) > 4 * x['Sales_box'] :
            suggest =   0
        elif (x['Suggestion_qty']+x['Inventory']) < 0.3 * x['Sales_box']:
            suggest =   x['Sales_box']
        else:
            suggest =   x['Suggestion_qty']
        return suggest

    data_merge['Suggestion_qty'] = data_merge.apply(lambda x: revised_suggest(x), axis=1)
